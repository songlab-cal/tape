...
class ConstantLRSchedule(LambdaLR):
    ...
    def __init__(self,
        ...
        super(ConstantLRSchedule, self).__init__(
            optimizer, lambda _: 1.0, last_epoch=last_epoch)  # type: ignore
...
class WarmupConstantSchedule(LambdaLR):
    ...
    def __init__(self,
        ...
        super(WarmupConstantSchedule, self).__init__(
            optimizer, self.lr_lambda, last_epoch=last_epoch)  # type: ignore
...
def get_scheduler(name: str) -> Type[LambdaLR]:
    name = name.lower()
    try:
        return LR_SCHEDULERS[name]
    except KeyError:
        raise KeyError(f"Unrecognized lr_scheduler {name}")
