...
TensorOrArray = typing.TypeVar("TensorOrArray", torch.Tensor, np.ndarray)
...
def http_get(url, temp_file, desc: typing.Optional[str] = None):
    req = requests.get(url, stream=True)
    content_length = req.headers.get("Content-Length")
    total = int(content_length) if content_length is not None else None
    progress = tqdm(unit="B", total=total, desc=desc)
...
def pad_sequences(
    sequences: typing.Sequence[TensorOrArray], constant_value=0, dtype=None
) -> TensorOrArray:
    ...
    if isinstance(sequences[0], np.ndarray):
        ...
        raise TypeError(f"Unrecognized type: {type(sequences[0])}")
